...
class reinforcement_net(nn.Module):
    '''Contains both the push and grasp nets
    Push interm, grasp interm, restricted interm
    Followed by two 1*1 conv layers with RELU activated inputs (last layer output is not RELU-activated)
    '''
    ...
    def __init__(self, use_cuda): # , snapshot=None
        ...
        #self.push_restricted_trunk = torchvision.models.densenet.densenet121(pretrained=True)
        #self.grasp_color_trunk = torchvision.models.densenet.densenet121(pretrained=True)
        #self.grasp_depth_trunk = torchvision.models.densenet.densenet121(pretrained=True)
        #self.grasp_restricted_trunk = torchvision.models.densenet.densenet121(pretrained=True)

        self.num_rotations = 1 #16
    ...
    def forward(self, input_color_data, input_depth_data, input_restricted_data, is_volatile=False, specific_rotation=-1):
        ...
        if is_volatile:
            with torch.no_grad():
                ...
                for rotate_idx in range(self.num_rotations):
                    ...
                    if self.use_cuda:
                        ...
                        rotate_restricted = F.grid_sample(Variable(input_restricted_data, volatile=True).cuda(), flow_grid_before, mode='nearest')
                    else:
                        ...
                        rotate_restricted = F.grid_sample(Variable(input_restricted_data, volatile=True), flow_grid_before, mode='nearest')
                    ...
                    interm_push_restricted_feat = self.push_restricted_trunk.features(rotate_restricted)
                    interm_push_feat = torch.cat((interm_push_color_feat, interm_push_depth_feat, interm_push_restricted_feat), dim=1)
                    interm_grasp_color_feat = self.grasp_color_trunk.features(rotate_color)
                    interm_grasp_depth_feat = self.grasp_depth_trunk.features(rotate_depth)
                    interm_grasp_restricted_feat = self.grasp_restricted_trunk.features(rotate_restricted)
                    interm_grasp_feat = torch.cat((interm_grasp_color_feat, interm_grasp_depth_feat, interm_grasp_restricted_feat), dim=1)
            ...
            if self.use_cuda:
                ...
                rotate_restricted = F.grid_sample(Variable(input_restricted_data, requires_grad=False).cuda(), flow_grid_before, mode='nearest')
            else:
                ...
                rotate_restricted = F.grid_sample(Variable(input_restricted_data, requires_grad=False), flow_grid_before, mode='nearest')
