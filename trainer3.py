...
GAMMA = 0.9 # 0.9 # 0.999 # Discount factor
...
TARGET_UPDATE = 10
TARGET_SAVE_CHECKPOINTS = [1, 500, 1000, 2000, 3000, 5000, 10000, 15000, 25000, 35000, 45000, 50000]
...
def get_reward3(prev_state, current_state):
    '''
    prev_state: (x1, y1, theta1, x2, y2, theta2)
    current_state: (x3, y3, theta3, _, _, _)
    '''
    reward = np.linalg.norm(prev_state[0:3] - prev_state[3:6]) - np.linalg.norm(current_state[0:3] - prev_state[3:6]) # prev distance - current distance
    return reward
...
def get_reward4(prev_state, current_state):
    ...
    reward = pos_diff + 0.05*orn_diff  #np.linalg.norm(prev_state[0:3] - prev_state[3:6]) - np.linalg.norm(current_state[0:3] - prev_state[3:6]) # prev distance - current distance
...
# plt.show()
run.finish()
# /home/vishal/Volume_E/Active/Undergrad_research/ICRA22/codebases/TEMP/Mid-Level-Planner/RL_manipulator_scripts